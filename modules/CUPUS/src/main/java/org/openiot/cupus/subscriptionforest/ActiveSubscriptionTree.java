 ...
 */
public class ActiveSubscriptionTree {
	
	public static final int ADDED_AS_PARENT = 13;

    private ActiveSubscriptionNode rootElement;
    ...
    public ActiveSubscriptionTree(ActiveSubscriptionNode rootNode) {
        this.addNode(rootNode, null); //will set this as new root
    }

    public int getTreeDepth(ActiveSubscriptionNode node) {
        int i = 0;
        if (!node.getChildren().isEmpty()) {
            int temp;
            for (ActiveSubscriptionNode e : node.getChildren()) {
     ...
     */
    protected void addAllSubscribers(ActiveSubscriptionTree other, boolean excludeRoot){
    ...
    private void findMatchingSubscribers(Publication publication, ActiveSubscriptionNode node, HashSet<UUID> subscribers){
    ...
    private void findMatchingSubscriptions(Announcement announcement, ActiveSubscriptionNode node, HashSet<Subscription> subscriptions){
     ...
     */
    public ActiveSubscriptionNode getRootElement() {
     ...
     */
    public void setRootElement(ActiveSubscriptionNode rootElement) {
    ...
    public List<ActiveSubscriptionNode> toList() {
        List<ActiveSubscriptionNode> list = new ArrayList<ActiveSubscriptionNode>();
     ...
     */
    private void walk(ActiveSubscriptionNode element, List<ActiveSubscriptionNode> list) {
        list.add(element);
        for (ActiveSubscriptionNode data : element.getChildren()) {
            walk(data, list);
        }
    }

    public int addNode(ActiveSubscriptionNode newNode, ActiveSubscriptionNode startNode) {
        ...
        if (newNode.data.getSubscription().coversSubscription(startNode.data.getSubscription())) {
            ...
            List<ActiveSubscriptionNode> toBeRemoved = new ArrayList<ActiveSubscriptionNode>();
            for (ActiveSubscriptionNode e : startNode.getChildren()) {
    ...
    public void removeNode(ActiveSubscription subscription, ActiveSubscriptionNode rootNode) {

        for (ActiveSubscriptionNode e : rootNode.getChildren()) {
            if (e.data.equals(subscription)) {
                for (ActiveSubscriptionNode f : e.getChildren()) {
    ...
    public void deleteSubscriber(UUID subscriberID) {
        ...
        for (ActiveSubscriptionNode e : this.toList()) {
    ...
    public boolean contains(Subscription sub, ActiveSubscriptionNode root) {
        if (this.rootElement.data.getSubscription().equals(sub)) {
            return true;
        } else {
            for (ActiveSubscriptionNode e : root.getChildren()) {
                contains(sub, e);
            }
        }
        return false;
