...
public class SensorScope2ServerWrapper extends AbstractWrapper {
    ...
    private static final String INITPARAM_SERVER_PORT = "server_port";
    ...
    SensorScope2Listener sensorScope2Listener = null;
    ...
    public boolean initialize() {
        ...
        // server port
        String str_server_port;
        if (addressBean.getPredicateValue(INITPARAM_SERVER_PORT) != null) {
            str_server_port = addressBean.getPredicateValue(INITPARAM_SERVER_PORT);
            logger.warn("SensorScope server port: " + str_server_port);
        } else {
            logger.warn("The specified parameter >" + INITPARAM_SERVER_PORT + "< for >SensorScope Server Wrapper< is missing.");
        ...
        if (addressBean.getPredicateValue(INITPARAM_STATION_ID) != null) {
            ...
            logger.warn("The specified parameter >" + INITPARAM_STATION_ID + "< for >SensorScope Server Wrapper< is missing.");
            logger.warn("Initialization failed.");
            return false;
        }
        ...
        server_port = Integer.parseInt(str_server_port);
    ...
    public String getWrapperName() {
        return "Sensorscope2 server wrapper";
    }
