...
public class kviMRproCol {
	...
	static class MapperInterTemp extends
	...
	static class MapperInterVal extends
		...
		public String modelContract(Result values, int order, double lb,
				double rb, double tl, double tr) {
			String res = "";

			res = Double.toString(tl) + ",";
			res += Double.toString(tr) + ",";
		...
		public void map(ImmutableBytesWritable row, Result values,
			...
			try {
				...
				lbstr= Bytes.toString(values.getValue(
						Bytes.toBytes("attri"), Bytes.toBytes("st")));
				rbstr = Bytes.toString(values.getValue(
						Bytes.toBytes("attri"), Bytes.toBytes("ed")));

				tbd[0] = Double.parseDouble(lbstr);
				tbd[1] = Double.parseDouble(rbstr);

				// ...................//

				model.set(modelContract(values, modOrd, bd[0], bd[1], tbd[0],
						tbd[1]));
				numMapIn++;
				cntnum++;
				// model.set(modelContract(values, modOrd, interv));
	...
	static class MapperPointTemp extends
			TableMapper<ImmutableBytesWritable, Text> {
	...
	static class MapperPointVal extends
			TableMapper<ImmutableBytesWritable, Text> {
...
//		scan.addColumn(Bytes.toBytes("attri"), Bytes.toBytes(col[1]));
		...
		if (col[0] == "st") {
			...
			TableMapReduceUtil.initTableMapperJob(idxtabname, scan,
					MapperPointTemp.class, ImmutableBytesWritable.class,
					Text.class, job);
			...
			job.setNumReduceTasks(1);
			TableMapReduceUtil.initTableMapperJob(idxtabname, scan,
					MapperPointVal.class, ImmutableBytesWritable.class,
					Text.class, job);
...
//		scan.addColumn(Bytes.toBytes("attri"), Bytes.toBytes(col[1]));
		...
		if (col[0] == "st") {
			TableMapReduceUtil.initTableMapperJob(idxtabname, scan,
					MapperInterTemp.class, ImmutableBytesWritable.class,
					Text.class, job);
			...
			TableMapReduceUtil.initTableMapperJob(idxtabname, scan,
					MapperInterVal.class, ImmutableBytesWritable.class,
					Text.class, job);
