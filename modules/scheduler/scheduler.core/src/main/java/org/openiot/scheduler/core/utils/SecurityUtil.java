...
public class SecurityUtil {
	 ...
	 *            <code>perm</code> on Scheduler. Otherwise, it is checked if <code>accessToken</code> has
		...
		if (credentials != null) {
			try {
				...
				if (acUtil.getAuthorizationManager().isCachingEnabled()) {
					// check if the Scheduler access token has expired (this step should be done only if
					// caching is enabled)
